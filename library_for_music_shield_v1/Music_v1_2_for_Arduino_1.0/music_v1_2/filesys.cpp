...
unsigned char OpenFile(unsigned int fileNumber)
{
  ...
  dirStack[dirLevel].sector=rootStart;///start sector of the root directory
  dirStack[dirLevel].entry=0;        ///point to the first direcctory entry
  ...
  while (fileNumber){
	...
	sectorAddress.l = 
	  ...
	  + diskSect.dir[temp.c].entry.FstClusLo;///get the correct start cluster of the file
