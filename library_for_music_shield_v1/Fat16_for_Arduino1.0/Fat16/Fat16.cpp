...
#include "Fat16.h"
//---------------------------------------------------------------------------
uint16_t listoffset=1;//
uint16_t* indexlist = 0;//index list of the valid directory entries of the songs
//-----------------------------------------------------------------------------
// volume info
...
//------------------------------------------------------------------------------
/*traverse the root directory and find the index of directory entries of 
the files with the name extention which fileNameExt point to.
The global pointer,indexlist point to the index 
*/
uint16_t Fat16::traverseDir(const char* fileNameExt)
{
  ...
  for (uint16_t index = 0; index < rootDirEntryCount_; index++) {
    ...
    else if((p->name[8] == *fileNameExt)&&(p->name[9] == *(fileNameExt + 1))&&(p->name[10] == *(fileNameExt + 2))){
      *(indexlist + listoffset) = index;
      listoffset ++;
    }
  }
}
 ...
 * Use Fat16::writeError to check for errors.
 */
size_t Fat16::write(const char* str) {
